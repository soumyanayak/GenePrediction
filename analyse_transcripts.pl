...
print "\nEnter the input file name (Trinity.fasta):\n";
...
for(my $i=0; $i < @line; $i++)
{
	if ($line[$i] =~ m/\>/)
	{
		...
		my $index = index($line[$i],$char);	#returns the position of first occurrence of white space character in the string
		my $scaf_name = substr($line[$i],1,$index-1);
	...
	#		$x++;
	#	}
		if($line[$i] !~ m/\>/)		#for calculating n50 of Bv_nuc.fasta
		{				#for calculating n50 of Bv_nuc.fasta
			$line[$i] == $seq;	#for calculating n50 of Bv_nuc.fasta
		}				#for calculating n50 of Bv_nuc.fasta
